...
def read_client_commands(client_id):
    filename = f"step4/clients/client_{client_id}.txt"
    commands = []
    ...
    while True:
        ...
        if has_started and repl_command != last_command:
            ...
            if parts[0] == "SPEED":
                ...
                if len(parts) == 1 or (len(parts) == 2 and int(parts[1]) == rank):
                    is_crashed = False
                    print(f"Client {rank}: Récupération effectuée.")
    ...
    while finished_clients < N_CLIENTS:
        ...
        if repl_command and repl_command != last_command:
            ...
            if parts[0] == "SPEED":
                ...
                if len(parts) == 1 or (len(parts) == 2 and int(parts[1]) == rank):
                    is_crashed = False
                    print(f"Serveur {rank}: Recovery effectué.")
                    for buffered_command in command_buffer:
                        log.append(buffered_command)
                    command_buffer = []
            last_command = repl_command
            ...
            for client in range(N_CLIENTS):
                ...
                if received_command == "FINISHED":
                    ...
                    if not is_crashed:
                        ...
                        command_buffer.append(received_command)
