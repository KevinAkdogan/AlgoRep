...
def is_valid_command(command):
    ...
    if parts[0] == "RECOVERY":
        if len(parts) == 1:
            ...
            if int(parts[1]) in crashed_processes:
                return True
            else:
                print(f"REPL: Processus {parts[1]} n'est pas en état de crash.")
                return False
    return False


def main():
    history_file = "step4/repl_history.txt"
    has_started = False
    while True:
        ...
        elif parts[0] == "RECOVERY":
            if len(parts) == 1:
                crashed_processes.clear()
                print("REPL: Tous les processus sont revenus dans le système.")
            else:
                crashed_processes.remove(int(parts[1]))
                print(f"REPL: Processus {parts[1]} récupéré.")
